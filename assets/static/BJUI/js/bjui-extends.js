            ...
            $.ajax({
                ...
                success  : function(response) {
                    ...
                    if (!json[BJUI.keys.statusCode]) {
                        ...
                        } else if(json[BJUI.keys.statusCode] == BJUI.statusCode.redirect) {
                            //新增，直接跳转功能 代码307
                            if(json.forward) {
                                if (json[BJUI.keys.message]) $this.alertmsg('ok', json[BJUI.keys.message])
                                $this.dialog('reload', {url:json.forward})
                            } else if (!$this.closest('.bjui-layout').length) {
                                if (json[BJUI.keys.message]) $this.alertmsg('error', json[BJUI.keys.message])
                                if ($this.closest('.navtab-panel').length) $this.navtab('closeCurrentTab')
                                else $this.dialog('closeCurrent')
                            }
