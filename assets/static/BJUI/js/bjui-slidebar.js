...
+function ($) {
    ...
    Slidebar.prototype.lock = function() {
        var that = this
        that.$window_shadow.addClass('show')
        that.$catalog_tip.addClass('show')
        that.$left_side.show().animate({left: 0}, 500)
        that.$new_lock.data('lock','on')
    }

    Slidebar.prototype.unlock = function() {
        var that = this
        that.$window_shadow.removeClass('show')
        that.$catalog_tip.removeClass('show')
        that.$left_side.animate({left: -230}, 500,function () {
            $(this).hide()
        })
        
        that.$new_lock.data('lock','off')
    }

    Slidebar.prototype._lock = function() {
        var that    = this
        var cwidth = BJUI.windowWidth - 236

        that.$navtab.animate({left:230, width:cwidth}, 400)
        that.$left_side.css('width','230px')
        that.$bar.animate({width: 230}, 500, function() {
    ...
    Slidebar.prototype._unlock = function() {
        var that    = this
        var cwidth  = BJUI.windowWidth - 56

        that.$navtab.animate({left:50, width:cwidth}, 400)
        that.$left_side.css('width','50px')
        that.$bar.animate({width: 50}, 500, function() {
    ...
    Slidebar.prototype._lg_menu = function () {
        var that = this
        var logo_box = that.$bar.find('.toggleCollapse')
        logo_box.find('.logo-min').hide()
        logo_box.find('.logo-lg').show()
        that.$bar.find('.sidebar-menu>li span').show()
    }

    Slidebar.prototype._min_menu = function () {
        var that = this
        var logo_box = that.$bar.find('.toggleCollapse')
        logo_box.find('.logo-lg').hide()
        logo_box.find('.logo-min').show()
        var item = that.$bar.find('.sidebar-menu>li')
        item.find('span').hide()
        that.$bar.one('click','.menu-items-tree',function () {
            that._lock()
        }).find('.menu-items-tree').removeClass('open').find('.menu-items').hide();
    }
    ...
    Slidebar.prototype.init = function() {
        ...
        this.$new_lock.on('click',function () {
                if(that.$new_lock.hasClass('sm-collapse')){//小屏
                    that.lock()
                }else{//大屏
                    if(that.$new_lock.data('lock') == 'on'){
                        that._unlock()
                    }else{
                        that._lock()
                    }
        ...
        this.$window_shadow.on('click',function () {
            that.unlock()
        })
