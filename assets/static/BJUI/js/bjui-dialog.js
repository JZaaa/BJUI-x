...
+function ($) {
    ...
    Dialog.DEFAULTS = {
        ...
        beforeClose : null,
        onClose     : null,
        current     : false //自定义,可选参数 navtab | dialog | false，是否在当前navtab/dialog下查找,用于解决非ajax调用多窗口id冲突问题
    }
        ...
        var tools = {
            ...
            reload: function($dialog, options) {
                ...
                if (options.url) {
                    ...
                    //解决dom调用页面id冲突
                    if(options.current){
                        if(options.current == 'navtab')
                            html = $.CurrentNavtab.find(options.target).html()
                        else if(options.current == 'dialog')
                            html = $dialog.find(options.target).html()
                    }else
    ...
    Dialog.prototype.open = function() {
        ...
        if ($dialog) { //if the dialog id already exists
            ...
            if (options.closeable) $dialog.find('a.close').show()
            else $dialog.find('a.close').hide()
    ...
    Dialog.prototype.reloadForm = function(clearQuery, option) {
        ...
        if ($dialog) {
            ...
            if ($pagerForm && $pagerForm.length) {
                ...
                //添加dialog查询构建选择，若存在checkbox请设置newquery参数为true
                if(options.newquery) {
                    options.data = data
                } else {
