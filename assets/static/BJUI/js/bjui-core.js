...
+function ($) {
    ...
    var BJUI = {
        ...
        ajaxTimeout: 30000,
        statusCode: {ok:200, error:300, timeout:301 ,warn : 302},
        ...
        initEnv: function() {
            ...
            setTimeout(function() {
                ...
                var $navbar = $('#bjui-navbar-collapse'),
                    ...
                    $navbar_cogs = $('#navbar-collapse-cogs')

                $navbar_cogs.on('click', function () {
                    $navbar.toggleClass('hidden')
                })
                $nav.on('click', '[data-toggle=navtab],[data-toggle=dialog]', function () {
                    if ($navbar_cogs.hasClass('show')) {
        ...
        initLayout: function(ww) {
            var iContentW = ww - (BJUI.ui.showSlidebar ? $('#bjui-sidebar').width() + 6 : 6),
                iContentH = $(window).height() - $('#bjui-footer').outerHeight(),
                navtabH   = $('#bjui-navtab').find('.tabsPageHeader').height() + $('#bjui-header').height()
            if (BJUI.ui.windowWidth) $('#bjui-window').width(ww)
            BJUI.windowWidth = ww
            var $left_side = $('#bjui-leftside'),
                $new_lock  = $('#menu_lock')
            //此处为左侧菜单适配
            if(BJUI.windowWidth < BJUI.ui.hideWidth){
                $left_side.hide().css('left',- $left_side.width() + 'px')
                $('#bjui-navtab').css('left',0).width(iContentW + $left_side.width())
                $new_lock.addClass('sm-collapse').data('lock','off')
            }else{
                $left_side.show().css('left','0')
                $('#bjui-navtab').css('left',$left_side.width()).width(iContentW)
                $new_lock.removeClass('sm-collapse').data('lock','on')
            }
            ...
            var navbarWidth = $('body').data('bjui.navbar.width'),
                $header = $('#bjui-header'), $toggle = $header.find('.bjui-navbar-toggle'), $logo = $header.find('.bjui-navbar-logo'), $navbar = $('#bjui-navbar-collapse'), $nav = $navbar.find('.bjui-navbar-right'),$navbar_collapse_cogs = $header.find('#navbar-collapse-cogs')
            if (!navbarWidth) {
            ...
            if (navbarWidth) {
                ...
                //顶部菜单适配
                if(ww - 300 < navbarWidth.navW ){
                    if(!$navbar.hasClass('hidden'))
                        $navbar.addClass('hidden open')
                    $navbar_collapse_cogs.addClass('show')
                }else{
                    $navbar.removeClass('hidden open')
                    $navbar_collapse_cogs.removeClass('show')
                }
            }
            /* horizontal navbar */
