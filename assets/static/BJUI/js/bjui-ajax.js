...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxDone = function(json) {
        ...
        if (json[BJUI.keys.statusCode] == BJUI.statusCode.error) {
            ...
            BJUI.loadLogin()
        } else if (json[BJUI.keys.statusCode] == BJUI.statusCode.warn) {
            if (json[BJUI.keys.message]) $element.alertmsg('warn', json[BJUI.keys.message])
        } else {
    ...
    Bjuiajax.prototype.ajaxCallback = function(json) {
        ...
        if (json[BJUI.keys.statusCode] == BJUI.statusCode.ok || json[BJUI.keys.statusCode] == BJUI.statusCode.warn ) {
    ...
    Bjuiajax.prototype.dialogCallback = function(json) {
        var that = this
        //修改处，当tabid为空时,200/302会刷新当前navtab,tabid=false时无刷新
        if((!json.tabid) && (json.statusCode == BJUI.statusCode.ok || json.statusCode == BJUI.statusCode.warn))
            setTimeout(function() { that.$element.navtab('refresh') }, 100)
        ...
        var search = function() {
            if ($target && $target.length) {
                ...
                if (that.tools.getTarget() == Bjuiajax.NAVTAB) {
                    ...
                    //拓展用于高级查询
                    if(options.searchmore){
                        $target = $.CurrentNavtab
                        form    = that.tools.getPagerForm($target, op)
                        $element.dialog('closeCurrent').navtab('reloadSearchMore', options.clearQuery, options, $element.serializeJson())
                    }else {
                        $target = $.CurrentDialog
                        form = that.tools.getPagerForm($target, op)
        ...
        if ($.validator) {
            $(document).on(BJUI.eventType.initUI, function(e) {
                $(e.target).find('form[data-toggle="ajaxsearch"]').each(function() {
                    ...
                    var $msgClass = options.msgClass?options.msgClass:"n-right"
