...
+function ($) {
    ...
    $(function() {
        var INIT_NAVTAB = function() {
            ...
            $main
                ...
                .click(function() { $(this).navtab('switchTab', 'main').navtab('refresh','main') })
                ...
                $panel
                    ...
                    .ajaxUrl({
                        type:(options.type || 'GET'), url:options.url, data:options.data || {}, loadingmask:options.loadingmask, callback:function(response) {
                            ...
                            //这里是
                            if(options.p_title){
                                $panel.find('.bjui-pageHeader>.pageTitle').empty().html(options.p_title + '>' + options.title)
                            }else{
                                $panel.find('.bjui-pageHeader>.pageTitle').empty().html(options.title)
                            }
                        }
                    })
            }
    ...
    //高级查询拓展功能
    Navtab.prototype.reloadSearchMore = function(clearQuery, option ,formData) {
        var options = $.extend({}, typeof option == 'object' && option)
        var $tab, $panel
        ...
        if ($tab && $panel) {
            if (!$tab.hasClass('external')) {
                ...
                if (clearQuery) {
                    //此处修复navtab清空查询对排序无效bug
                    formData.orderDirection = formData.orderField = ''
                    $panel.data('bjui.clientPaging').orderDirection = $panel.data('bjui.clientPaging').orderField = ''
                }
